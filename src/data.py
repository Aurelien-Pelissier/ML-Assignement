...
class Imagenet64(object):
    def __init__(self, data_path):
        ...
        with open(self.data_path / "dev_data/dev_data_batch_1", "rb") as fo:
            ...
            y_val = np.array(data["labels"]) - 1

        
        #Plot Database statistics
        print('Database statistics:')
        
        print('  N_imgs (train): ', len(y_train))
        print('  N_imgs (val) : ', len(y_val))
        
        print('  N_train_classes : ', len(np.unique(y_train)))
        print('  N_val_classes : ', len(np.unique(y_val)))
        ...
        assert (
            ...
            len(np.unique(self.data["y_train"])) >= len(np.unique(self.data["y_val"]))
        )
...
if __name__ == "__main__":
    ds = Imagenet64("data")
    dg = ds.datagen_cls(10, augmentation=False)
